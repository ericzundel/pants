...
public class ConsoleRunnerImpl {
  ...
  void run(Collection<String> tests) {
    ...
    try {
      ...
      if (useExperimentalRunner) {
        failures = run_experimental(parsedTests, core);
      } else {
        failures = run_legacy(parsedTests, core);
      }
  ...
  private int run_experimental(List<Spec> parsedTests, JUnitCore core)
  ...
  private int run_legacy(List<Spec> parsedTests, JUnitCore core) throws InitializationError {
