...
class BuildFile(object):
  ...
  @staticmethod
  def scan_buildfiles(root_dir, base_path=None, excludes=None):
    ...
    :param excludes: list of absolute paths to exclude from the scan"""
    ...
    def remove_excluded(root, dirs, exclude_roots):
      ...
      for exclude_root in exclude_roots:
        ...
        if root.rstrip('/') == exclude_root:
          for dir in exclude_roots[exclude_root]:
            if dir in dirs:
    ...
    if not excludes:
      ...
      exclude_roots = calc_exclude_roots(root_dir, excludes)

    for root, dirs, files in os.walk(os.path.join(root_dir, base_path or ''), topdown=True):
      remove_excluded(root, dirs, exclude_roots)
