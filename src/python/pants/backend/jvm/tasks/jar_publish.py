  ...
  def register_options(cls, register):
    ...
    register('--repos', advanced=True, type=dict,
             help='Settings for repositories that can be pushed to. See '
                  'https://www.pantsbuild.org/publish.html for details.')
    register('--publish-extras', advanced=True, type=dict,
             help='Extra products to publish. See '
                  'https://www.pantsbuild.org/dev_tasks_publish_extras.html for details.')
    ...
    if self.get_options().local:
      ...
      if not self.repos:
        raise TaskError(
          ...
          "http://www.pantsbuild.org/publish.html#authenticating-to-the-artifact-repository,\n"
