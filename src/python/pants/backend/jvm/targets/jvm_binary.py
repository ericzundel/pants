...
class Duplicate(JarRule):
  ...
  def fingerprint(self):
    ...
    hasher.update(repr(self._action))  # <object object at 0x...>
    return hasher.hexdigest()
...
class JarRules(FingerprintedMixin, object):
  ...
  def fingerprint(self):
    hasher = sha1()
    hasher.update(repr(self.default_dup_action))
